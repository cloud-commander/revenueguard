...
export const EdgePresence = ({ mode }: { mode: SimulationMode }) => {
  return (
    <div className="space-y-2">
      ...
      <div className="relative h-28 bg-muted/30 rounded-lg border border-border/50 p-4 flex flex-col overflow-hidden">
        {/* Tier 1: Icons & Track */}
        ...
        {/* Tier 2: Distance/Latency Labels (Aligned with Track) */}
        <div className="h-1/2 flex items-end pb-1 relative gap-4 px-2">
          {/* 1. Leading Spacer (matches LHR icon width) */}
          <div className="w-12 shrink-0" />

          {/* 2. Label Container (Centred over Track) */}
          <motion.div
            layout
            className={cn(
          ...
          >
            <Tooltip>
              <TooltipTrigger asChild>
                <motion.div
                  layout
                  className="z-30 flex flex-col items-center gap-0.5 bg-background/90 backdrop-blur-sm px-3 py-1 rounded-lg border border-border shadow-sm min-w-[140px] cursor-help"
                >
                  ...
                  <span className="text-[7px] font-bold uppercase text-muted-foreground whitespace-nowrap leading-none mt-0.5">
                    {mode === "eventual"
          ...
          {/* 3. Trailing Spacers (Mirror Tier 1 to prevent overlap with JFK/Slow) */}
          <div className="flex flex-col items-center gap-1 z-10 shrink-0 opacity-0 pointer-events-none">
            <div className="w-8 h-8" />
            <span className="text-[10px] font-bold">JFK (ORIGIN)</span>
          </div>
          <motion.div
            layout
            className={cn(
